...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        grid_eps: float = 0.02,
        grid_range: list[float] | tuple[float, float] = (-1, 1),
    ) -> None:
        super().__init__()
    ...
    def curve2coeff(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        a_matrix = self.b_splines(x).transpose(0, 1)
        b_matrix = y.transpose(0, 1)
        solution = torch.linalg.lstsq(a_matrix, b_matrix).solution
